 ...
 function myFunc1() {
     var x = document.getElementById("myTopnav"),
         xx = document.getElementsByClassName('glyphicon-triangle-bottom')
     x.className = "topnav";
     xx.className = ''
 ...
 function callMapboxDiff() {
     ...
     let markLat = [816459, 831891, 822882, 818986, 814906, 797246, 799846, 790364, 802545, 810918, 828376, 801346, 8083, 741856, 864379]
     let markLon = [460835, 435944, 449848, 294838, 448732, 42573, 436177, 409937, 440793, 44767, 491916, 439154, 488526, 319353, 381114]
     let urlW = ['Republic_Square_(Belgrade)', 'Great_War_Island', 'Belgrade_Fortress', 'Belgrade_Nikola_Tesla_Airport', 'Branko%27s_Bridge', 'Ada_Bridge', 'New_Railway_Bridge', 'Ada_Ciganlija', 'Gazela_Bridge', 'Old_Sava_Bridge', 'Pan%C4%8Devo_Bridge', 'Old_Railway_Bridge', 'Belgrade_New_Cemetery', 'Ostru%C5%BEnica_Bridge', 'Pupin_Bridge'];
     let lenMark = markLat.length;
     for (let r = 0; r < lenMark; r++) {
         L.marker(['44.' + markLat[r], '20.' + markLon[r]]).addTo(mymap1).bindPopup(`<br><b><a href='https://en.wikipedia.org/wiki/${urlW[r]}' target='_blank'>${decodeURI(urlW[r].split('_').join(' '))}</a></b>`)
     }
     L.circle([44.7950478, 20.4394765, 17.71], {
         color: '#8CB240',
         fillColor: '#8CB240',
 ...
 fetch('https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20rss%20where%20url%20in%20(%27https%3A%2F%2Fwww.blic.rs%2Frss%2FVesti%2FBeograd%27)&format=json&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys')
     ...
     .then(blicRSS => {
         for (let i = 0; i <= 41; i++) {
             ...
             var dT1 = d.pubDate,
                 len = dT1.length;
             var datP1 = dT1.slice(0, len - 23),
                 datP2 = dT1.slice(len - 23, len - len - 15);
             $('#myCarousel').carousel({
                 interval: 5000
             });
